    ...
    <div class="container top">
        <div class="row">
            <div class="col-12 col-md-12 col-lg-9 col-xl-9">
                ...
                <p class="lead mb-4">Questo post piace a <span
                        class="text-primario font-weight-bold">{{ $post->likesCount() }}</span> persone</p>
                <a class="btn-like py-2 px-3 mb-5" href="{{ route('post.like', compact('post')) }}"><i
                    ...
                    @if (Auth::user()->role == 'admin' || $post->user_id == Auth::user()->id)
                        ...
                        <div class="d-flex justify-content-start mt-5">
                            ...
                            <a href="{{ route('post.edit', ['post' => $post]) }}"
                                class="text-decoration-none p-3 btn-edit ml-3">Modifica post</a>
                        </div>
            ...
            @if (is_array($relatedPosts))
                <x-sidebar :post="$post" :related="$relatedPosts" />
            @else
                <x-sidebar :post="$post" related='' />
            @endif
